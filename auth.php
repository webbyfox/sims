 ...
 */
// class auth_plugin_sims extends auth_plugin_base {
...
//         $this->authtype = 'sims';
//         $this->config = get_config('auth/sims');
...
//      */
//     public function auth_plugin_sims() {
...
//        ChromePhp::log('Hello Riz console!');
//        ChromePhp::log($username);
//        ChromePhp::log($password);
//        //ChromePhp::warn('something went wrong!');

//         // $to = "rmansuri@rcseng.ac.uk";
//         // $subject = "My subject";
//         // $txt = "Hello world!";
//         // $headers = "From: rmansuri@rcseng.ac.uk";
//         //
//         // mail($to,$subject,var_dump($username),$headers);
//         // var_dump($username);
...
//         ChromePhp::log($extusername);
//         ChromePhp::log($extpassword);
...
//             if (!$rs) {
//                 $authdb->Close();
//                 debugging(get_string('auth_simcantconnect','auth_sims'));
//                 return false;
//             }
...
//             if (!$rs) {
//                 $authdb->Close();
//                 debugging(get_string('auth_simscantconnect','auth_sims'));
//                 return false;
//             }
...
//             } else if ($this->config->passtype === 'sims_sha512') {
//                 return ($fromdb == $this->CreatePasswordHash($extpassword, $salt)); //SIMS custom auth
...
class auth_plugin_sims extends auth_plugin_base {
    ...
    function user_login($username, $password) {
        ...
        require_once($CFG->dirroot .'/chromephp-master/ChromePhp.php');
        ChromePhp::log('Hello Riz console!');
        ChromePhp::log($username);
        ChromePhp::log($password);
    ...
    function db_init() {
        if ($this->is_configured() === false) {
            throw new moodle_exception('auth_simscantconnect', 'auth_sims');
        }
    ...
    function sync_users(progress_trace $trace, $do_updates=false) {
        ...
        if (!empty($this->config->removeuser)) {
            ...
            if (!empty($removeusers)) {
                $trace->output(get_string('auth_simsuserstoremove', 'auth_sims', count($removeusers)));

                foreach ($removeusers as $user) {
                    if ($this->config->removeuser == AUTH_REMOVEUSER_FULLDELETE) {
                        delete_user($user);
                        $trace->output(get_string('auth_simsdeleteuser', 'auth_sims', array('name'=>$user->username, 'id'=>$user->id)), 1);
                    } else if ($this->config->removeuser == AUTH_REMOVEUSER_SUSPEND) {
                        $updateuser = new stdClass();
                        $updateuser->id   = $user->id;
                        $updateuser->suspended = 1;
                        user_update_user($updateuser, false);
                        $trace->output(get_string('auth_simssuspenduser', 'auth_sims', array('name'=>$user->username, 'id'=>$user->id)), 1);
                    }
                }
            }
            unset($removeusers);
                ...
                if ($update_users) {
                    ...
                    foreach ($update_users as $user) {
                        if ($this->update_user_record($user->username, $updatekeys)) {
                            $trace->output(get_string('auth_simsupdatinguser', 'auth_sims', array('name'=>$user->username, 'id'=>$user->id)), 1);
                        } else {
                            $trace->output(get_string('auth_simsupdatinguser', 'auth_sims', array('name'=>$user->username, 'id'=>$user->id))." - ".get_string('skipped'), 1);
                        }
                    }
                    unset($update_users);
                }
        ...
        if (!empty($add_users)) {
            $trace->output(get_string('auth_simsuserstoadd','auth_sims',count($add_users)));
            ...
            foreach($add_users as $user) {
                ...
                if ($this->config->removeuser == AUTH_REMOVEUSER_SUSPEND) {
                    if ($olduser = $DB->get_record('user', array('username' => $username, 'deleted' => 0, 'suspended' => 1,
                        ...
                        $trace->output(get_string('auth_simsreviveduser', 'auth_sims', array('name' => $username,
                            'id' => $olduser->id)), 1);
                        continue;
                    }
                }
                ...
                if ($collision = $DB->get_record_select('user', "username = :username AND mnethostid = :mnethostid AND auth <> :auth", array('username'=>$user->username, 'mnethostid'=>$CFG->mnet_localhost_id, 'auth'=>$this->authtype), 'id,username,auth')) {
                    $trace->output(get_string('auth_simsinsertuserduplicate', 'auth_sims', array('username'=>$user->username, 'auth'=>$collision->auth)), 1);
                    continue;
                }
                try {
                    $id = user_create_user($user, false); // It is truly a new user.
                    $trace->output(get_string('auth_simsinsertuser', 'auth_sims', array('name'=>$user->username, 'id'=>$id)), 1);
                } catch (moodle_exception $e) {
                    $trace->output(get_string('auth_simsinsertusererror', 'auth_sims', $user->username), 1);
                    continue;
                }
    ...
    function user_exists($username) {
        ...
        if (!$rs) {
            print_error('auth_simscantconnect','auth_sims');
        } else if (!$rs->EOF) {
    ...
    function update_user_record($username, $updatekeys=false) {
        ...
        if (empty($user)) { // trouble
            ...
            print_error('auth_simsusernotexist','auth_sims',$username);
            die;
        }
